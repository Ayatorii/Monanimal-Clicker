          ...
          {ACHIEVEMENTS.map(ach => {
            ...
            return (
              <div key={ach.id} className={cn(
                "p-3 rounded-md border flex items-center gap-4 transition-colors", 
                unlocked ? "border-primary bg-primary/5" : "border-border opacity-60 bg-muted/30"
              )}>
                <div className="text-3xl bg-background rounded-md p-2 shadow-inner border border-border flex items-center justify-center w-14 h-14">
                  {unlocked ? ach.icon : <Lock className="h-6 w-6 text-muted-foreground" />}
                </div>
                <div className="flex-1">
                  <div className={cn("font-bold text-sm uppercase tracking-wide", unlocked ? "text-primary" : "text-muted-foreground")}>
                    {ach.name}
                  </div>
